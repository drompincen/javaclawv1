...
public class CreatePhaseTool implements Tool {
    ...
    public ToolResult execute(ToolContext ctx, JsonNode input, ToolStream stream) {
        ...
        var existing = thingService.findByProjectCategoryAndNameIgnoreCase(projectId, ThingCategory.PHASE, name);
        if (existing.isPresent()) {
            ObjectNode result = MAPPER.createObjectNode();
            result.put("phaseId", existing.get().getId());
        ...
        payload.put("sortOrder", input.path("sortOrder").asInt(1));
        payload.put("status", PhaseStatus.PENDING.name());

        if (input.has("entryCriteria") && input.get("entryCriteria").isArray()) {
            List<String> criteria = new ArrayList<>();
            input.get("entryCriteria").forEach(n -> criteria.add(n.asText()));
            payload.put("entryCriteria", criteria);
        }
        if (input.has("exitCriteria") && input.get("exitCriteria").isArray()) {
            List<String> criteria = new ArrayList<>();
            input.get("exitCriteria").forEach(n -> criteria.add(n.asText()));
            payload.put("exitCriteria", criteria);
        }

        String startDate = input.path("startDate").asText(null);
        if (startDate != null) payload.put("startDate", Instant.parse(startDate + (startDate.contains("T") ? "" : "T00:00:00Z")).toString());
        String endDate = input.path("endDate").asText(null);
        if (endDate != null) payload.put("endDate", Instant.parse(endDate + (endDate.contains("T") ? "" : "T00:00:00Z")).toString());

        payload.put("checklistIds", List.of());
        payload.put("objectiveIds", List.of());

        var thing = thingService.createThing(projectId, ThingCategory.PHASE, payload);
        stream.progress(100, "Phase created: " + name);

        ObjectNode result = MAPPER.createObjectNode();
        result.put("phaseId", thing.getId());
        result.put("name", name);
        return ToolResult.success(result);
    }
}
