...
PROJECT=$(curl -s -X POST "$BASE_URL/api/projects" \
  ...
  -d '{"name":"Tutorial Sprint Planning","description":"Sprint objectives and capacity demo","tags":["tutorial","sprint"]}')
...
echo "$OBJECTIVES" | jq -r '.[] | "  [\(.status)] \(.outcome) — coverage: \(.coveragePercent)%"'
...
echo "    - Evidence Service objective is ACTIVE with 3 linked tickets"
