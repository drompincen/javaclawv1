...
curl -s -X POST "$BASE_URL/api/projects/$PROJECT_ID/threads" \
  -H 'Content-Type: application/json' \
  -d '{"title":"Evidence Service Refactor","summary":"Splitting monolith into strategy pattern. Joe owns refactor, 3 handlers. Target: Sprint 42."}' > /dev/null
ok "Thread: Evidence Service"

# Seed a ticket
curl -s -X POST "$BASE_URL/api/projects/$PROJECT_ID/tickets" \
  -H 'Content-Type: application/json' \
  -d '{"title":"KYC-103: Refactor bank statement handler","description":"Joe, IN_PROGRESS, 3 SP","priority":"HIGH"}' > /dev/null
ok "Ticket: KYC-103"
